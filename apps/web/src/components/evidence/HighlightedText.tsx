 ...
 * evidence_snippet 就丢掉了，`mentions` 表**没有这两列**（docs/28 §2.4）。
 *
...
export function HighlightedText({
  ...
  if (highlights.length === 0) {
    return <div className={styles.answer}>{text}</div>
  }
  ...
  for (const [i, h] of sorted.entries()) {
    ...
    if (h.offset > cursor) parts.push(<Fragment key={`t${i}`}>{text.slice(cursor, h.offset)}</Fragment>)
    const end = h.offset + h.matchedTerm.length
    parts.push(
      <mark key={`h${i}`} className={styles.hl}>
