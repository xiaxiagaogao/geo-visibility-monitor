...
绕过我们自己域名上的 Bot 保护，而且慢一个数量级、截图上传要 16 秒。

所以 **`GEO_API_BASE` 留在 tailnet**。真要走公网的话，正确做法是在 Cloudflare
上给 `/v1/worker/*` 加一条 WAF skip 规则 —— **在 CF 那边开门，不在这边伪装。**
在那之前，「新节点只需要一把 key + 一个公网地址」这句话**还不成立**。
...
### 4. 截图：隧道模式下关着，HTTP worker 模式下回来了

`api` 和 `crawler` 原本共享一个 volume，截图靠它工作。crawler 搬走之后，
写在节点上的截图 VPS 读不到 —— **留着只会让证据页 404**。

所以**隧道模式**下 `SCREENSHOT_DIR=` 置空，证据页那个「查看截图」按钮条件渲染、
直接不显示。**降级是干净的。**

**P2-34 的 HTTP worker 模式下这笔债已经还上**（2026-08-18）：截图随结果
`POST /v1/worker/jobs/{id}/result` 的 multipart 传回 VPS，由 api 落到它那边的
`SCREENSHOT_DIR`。所以切到 worker 模式时**要把节点的 `SCREENSHOT_DIR` 设回一个
本地目录**（容器内 `/data/screenshots`），否则 provider 根本不截图，也就没什么可传。

> 两种模式对这个变量的要求正好相反，切换时别照抄另一种的配置：
> **隧道模式必须为空，worker 模式必须非空。**
