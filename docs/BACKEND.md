...
| 丢弃 | 原因 |
|------|------|
| B1–B7 逐步日记、重复验收表 | 已完成；价值已吸收为约束与 API |
| 本机 Docker 为主路径的叙述 | 现行是 VPS 运行 |
| L3 页面/IA/g1geo 排版长文 | 属前端产品，不归后端核心文档 |
| Playwright 通用教程全文 | 只保留「我们怎么取证」的约束 |
| Git 教程长文 | 运维保留 push 必要命令即可 |
| 开源项目大表、头脑风暴原文 | 背景材料，非现行规格 |
| counts 的 category 参数设想 | **已拍板不做** |
| 把 rate 写入 metric_snapshots 当权威 | 与分层冲突 |
| 错误清洗写回 L0 的旧描述 | 已修复为原文优先 |
