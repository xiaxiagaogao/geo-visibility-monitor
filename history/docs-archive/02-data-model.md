...
| 表 / 能力 | 层级 | 说明 |
|-----------|------|------|
| `raw_responses` / `citations` | **L0** | 原文与引用 |
| `mentions`（及 answer_status 等标注字段） | **L1** | 单次标注；分母与命中事实 |
| 计数查询（group by） | **L2** | MVP **API 聚合**；日后可物化日表 |
| 比率 / 综合分 | **L3** | **不落库**（前端算） |
| `metric_snapshots` | 可选缓存 | 若存必须是 counts 或带版本的展示快照，默认不用 |
